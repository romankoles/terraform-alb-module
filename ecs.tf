...
module "my-ecs" {
  source         = "github.com/romankoles/terraform-modules//modules/ecs-cluster?ref=terraform-0.12"
...
module "my-service" {
  source              = "github.com/romankoles/terraform-modules//modules/ecs-service?ref=terraform-0.12"
  VPC_ID              = module.vpc.vpc_id
...
module "my-alb" {
  source             = "github.com/romankoles/terraform-modules//modules/alb?ref=terraform-0.12"
  VPC_ID             = module.vpc.vpc_id
  ALB_NAME           = "my-alb"
  VPC_SUBNETS        = join(",", module.vpc.public_subnets)
  DEFAULT_TARGET_ARN = module.my-service.target_group_arn
  DOMAIN             = "*.ecs.aibroonie.com"
  INTERNAL           = false
